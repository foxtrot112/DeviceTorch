...
void matrix_load_cache(std::string name, std::string location, matrix *mat)
{
    ...
    // Read data
    size_t read_count = fread(mat->data, sizeof(float), rows * cols, f);
...
void matrix_outer_product(matrix* a, matrix* b, matrix* result)
{
    ...
    for (int i = 0; i < a->rows; i++) {
        for (int j = 0; j < b->rows; j++) {
            float val = matrix_fetch(a, i, 0) * matrix_fetch(b, j, 0);
            matrix_store(result, i, j, val);
        }
    }
}
...
void matrix_multiply_transpose(matrix* A, matrix* B, matrix* result)
{
    ...
    for (int i = 0; i < A->cols; i++) {          // columns of A = rows of A^T
        float sum = 0.0f;

        for (int j = 0; j < A->rows; j++) {     // rows of A
            float a = matrix_fetch(A, j, i);     // NOTE swapped indices (transpose)
            float b = matrix_fetch(B, j, 0);

            sum += a * b;
        }

        matrix_store(result, i, 0, sum);
    }
}
...
void matrix_scale(matrix* A, float scalar)
{
    for (int i = 0; i < A->rows; i++) {
        for (int j = 0; j < A->cols; j++) {
            ...
            float val = matrix_fetch(A, i, j);
            matrix_store(A, i, j, val * scalar);
        }
    }
}